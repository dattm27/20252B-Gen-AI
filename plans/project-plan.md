...
- [x] **Cập nhật (Hoàng, 26/07): tách riêng "đúng sự thật" khỏi "đủ nội dung" trong factual checker** ("Separate factuality from report coverage"). `check_reasoned_report` giờ trả 2 cờ độc lập: `factual_passed` (mọi con số/reason đã nhắc tới có khớp nguồn không — tiêu chí hallucination) và `coverage_passed` (đủ cả 4 aspect bắt buộc không — tiêu chí độ đầy đủ), cộng `repeated_aspects`/`coverage_warnings`. Có 2 mode: `factual-only` (mặc định, `passed = factual_passed`, thiếu/lặp aspect chỉ là cảnh báo) và `strict` (`passed` cần cả 2 cờ true). Lý do: bỏ sót 1 aspect nhưng số liệu đã nói đều đúng an toàn hơn nhiều so với bịa số liệu — không nên coi hai loại lỗi này nghiêm trọng như nhau. Test mới: `test_reasoned_checker_separates_factuality_from_coverage` (tests/test_report_generation.py, 17 test tổng).

  **Chạy thêm trên demo Yelp** (`output/flan_t5_yelp_reasoned_report.json`, input `aspect_reasons_yelp_demo.json`): chọn 4 aspect `ice cream`/`atmosphere`/`parking`/`line`, report chỉ thực sự nhắc 3/4 (bỏ sót `line`, lặp `parking`) — **factual_passed: true** (3/3 claim thực sự đưa ra đều khớp nguồn: ice cream 765/621, atmosphere 64/66, parking cả 2 lần nhắc), **coverage_passed: false**, nhưng `passed: true` (mode `factual-only`) với `status: accepted_with_warnings`. Đây là lượt đầu tiên chạy full pipeline (ASTE → tổng hợp → sinh report → factual check) trên chính dữ liệu demo quy mô lớn, không chỉ dừng ở bảng thống kê.

  Bảng thống kê restaurant chạy lại với checker mới cũng cho thấy `coverage_passed: true` nhưng có `repeated_aspects: [dessert, indian food]` (report nhắc 2 aspect này 2 lần) — cảnh báo đúng, không phải lỗi số liệu.

  Đã cập nhật `report/main.tex` Mục 4.5 (mô tả 2 cờ + 2 mode), Mục 5.4 (kết quả cả 2 demo), Mục 7 (hạn chế: ranh giới factual/coverage là lựa chọn thiết kế có thể tranh luận).
...
- [x] Thêm baseline neural (không generative) để so sánh — **GTS-BERT**, cài lại theo Wu et al. (Findings of EMNLP 2020, "Grid Tagging Scheme for Aspect-oriented Fine-grained Opinion Extraction"). BERT-base-uncased + MLP phân loại mọi cặp token trong câu thành lưới $n\times n$ (nhãn N/A/O/POS/NEU/NEG), giải mã bằng đọc span trên đường chéo + biểu quyết đa số sentiment. Logic xây/giải mã lưới thuần Python, test local trước (`tests/test_gts_grid.py`, 10 test, có test tái hiện đúng ví dụ minh hoạ của bài báo gốc), inline vào notebook Kaggle để train (`notebooks/train-gts-bert-for-aste-on-14res-15res-16res.ipynb`, kết quả `notebooks-output/gts-bert-baseline-for-aste-on-14res-15res-16re-output.ipynb`).

  **Kết quả** (cùng test split 1134 câu, 14res+15res+16res, giống hệt 3 model T5): Test triplet-F1 = **0.6920** (P=0.7088, R=0.6760); dev triplet-F1 hội tụ ổn định qua 20 epoch (0.27→0.73, không dấu hiệu phân kỳ). Xếp vị trí: AsteLookupBaseline (0.3736) < GTS-BERT (0.6920) < FLAN-T5-base (0.5898 -- **thấp hơn GTS-BERT**) < T5-small (0.7240) < T5-base (0.7442). Insight: bản thân dùng neural/BERT không đảm bảo vượt qua baseline generative — cách đóng khung bài toán (sinh trực tiếp bộ ba dạng text-to-text vs. phân loại lưới rồi giải mã) mới là yếu tố chính giúp T5 vượt trội. Đã thêm vào `report/main.tex` (Mục 4.1, bảng so sánh Mục 5.1, bibliography).
